...
if(  $('.items ul li:nth-child('+1+')').hasClass('current') )
    {
		console.log("БЛЯТЬ");
...
sli.style.backgroundImage = 'url(img/nn.png)';
	 }
  $('.sl'+slide_prev).addClass('hidden');
...
$('.show-modal-window-form').click(function(){	$(".b-modal-window.m-form").css("display", "block");	});
	...
	'&price_contact='+$('.raschet .one_contact').html()+'&monitors='+$('.raschet .ekranu span').text()+'&howmany='+$('.raschet .count.chastotnost').text()+
...
$(".sl10 input[type=checkbox]").click( function(e) {
	...
	switch(x) {
	  ...
	  if (muha>3)
	  {
		  alert("Для вас будет выгодней выбрать пакет Вся Казань");
		 
		  $('#radio22').prop('checked', false);
		  $('#radio23').prop('checked', false);
		  $('#radio24').prop('checked', false);
		  slide.style.backgroundImage = 'url(img/map.png), url(img/kirov.png)';
		  muha=1;
		  pid1='';
		  pid2='';
		  pid3='';
...
function sl4_skidka(chastota){
	...
	if(sec == 10){
		...
		if(chastota == 3){
			$('.text_procent2').html('16%');
			hrch=16;
		}
		if(chastota == 4){
			$('.text_procent2').html('17%');
			hrch=17;
		}
		if(chastota == 5){
			$('.text_procent2').html('20%');
			hrch=20;
		}
	}
	if(sec == 15){
...
$(function() {
  $('#date_period').datepicker({
    ...
    $('.period span').html(daysLag + ' дней');
	  ...
	  $('#period_month').html('Период размещения <br> <span>suka pidor</span>');
...
function chastotnost(chast, period, ekranu){
  console.log("Хуясть"+chast);
	sytki = 18*60*60/300*chast;
	...
	kontaktu = (54/5*parseInt(kol_ekranov)*window.counts * parseInt(kol_dney)) *1000* coef;
    kontaktu = kontaktu.toFixed(0);
...
function totalPrice(){
	...
	old_price = 0.4*hron*288*kol_dney*kol_rk*chust; //расчет старого прайса
  ...
  var count_see = (18*60*60/300*parseInt($('.chast_pokaza').val()));
